...
def archive_old_operations(sender, request, user, **kwargs):
    ...
    p_operations = PlaceholderOperation.objects.filter(user=request.user, site=site).exclude(
...
def update_placeholder_operation(sender, **kwargs):
    ...
    p_operations = PlaceholderOperation.objects.filter(
        site=site, user=request.user, user_session_key=request.session.session_key
    )
    ...
    p_operations.filter(~Q(origin=kwargs["origin"]) | Q(origin=kwargs["origin"], is_applied=False)).update(
        is_archived=True
    )
    ...
    foreign_operations = PlaceholderOperation.objects.filter(origin=kwargs["origin"], site=site).exclude(
        user=request.user
    )
