...
class UndoRedoToolbar(CMSToolbar):
    ...
    class Media:
        css = {"all": [static("djangocms_history/color_mode.css")]}
    ...
    def _get_ajax_button(self, name, url, icon, button_type, disabled=True):
        ...
        button = AjaxButton(
            name=name, url=url, data=data, icon="", active=False, disabled=disabled, button_type=button_type
        )
        return button
    ...
    def get_undo_button(self):
        ...
        button = self._get_ajax_button(name=gettext("Undo"), url=url, icon="", disabled=disabled, button_type="undo")
        return button
